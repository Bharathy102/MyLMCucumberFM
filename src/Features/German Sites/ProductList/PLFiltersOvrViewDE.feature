@productlistde @de @ploverfilters
...
Scenario: Verify ProductList filters on Overview Tab
	...
	Then I verify the chip is displayed for the selected filter
	Then I verify the filtered results on "tab-overview" tab based on the "share-classes-fields" and "A" selected filters on "2" position for German Site
	Then I verify the filtered results on "tab-overview" tab based on the "asset-class-fields" and "Equity Income" selected filters on "4" position for German Site
	...
	Then I verify the filtered results on "tab-overview" tab based on the "checkbox-group distribution-type" and "Accumulating" selected filters on "5" position for German Site
	And I select the first Distribution Type filter checkbox
	And I select the "EUR" Currency filter checkbox for German Site
	Then I verify the filtered results on "tab-overview" tab based on the "currency-fields" and "EUR" selected filters on "3" position for German Site
	And I select the "EUR" Currency filter checkbox for German Site
